...
export const attendanceApi = {
  ...
  getByDate: async (date: string) => {
    return apiRequest<any[]>(`/attendance?date=${date}`);
  },

  markBulk: async (date: string, records: Array<{ studentId: number; status: string }>) => {
    return apiRequest<{ ok: boolean; count: number }>('/attendance/bulk', {
      method: 'POST',
